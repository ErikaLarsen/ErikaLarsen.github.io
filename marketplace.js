...
function loadMap(mapSpec){
    ...
    if(mapSpec == maps.uscTemperature){
        ...
        if(objectKey == "sensors"){
            ...
            description = 'Facilities Management<br>Services Vehicle';
        }
    ...
    for(i=0;i<mapSpec.areas.length;i++){
        ...
        addMapArea(map,maps[mapSpec.areas[i].load],mapSpec.areas[i],i);   
    }
}
...
function addMapArea(map, mapSpec, area, i ){
    ...
    google.maps.event.addListener(shape, 'click', function (event) {
        if(area.product){
            $.getJSON('https://data.lacity.org/resource/xzkr-5anj.json', function(data) {
            //data is the JSON string
                console.log(data);
                var date = JSON.stringify(data[i].dataexportdatetime);
